...
After the first load, the app works fully offline — all static assets are precached by the Workbox service worker generated by `vite-plugin-pwa`. API responses are cached with appropriate TTLs (10 min for today's forecast, 1 h for archive data, 24 h for geocoding).
